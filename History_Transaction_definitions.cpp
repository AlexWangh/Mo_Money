...
void History::sort_by_date(){
  ...
  Transaction *pre_node=nullptr,*pre_key=nullptr,*key=nullptr,*post=nullptr;
  if(node==nullptr) return;
  pre_node=node;
  node=node->get_next();
  while(node!=nullptr){
    post=node->get_next();
    //1.find the key postion
    key=p_head;
    pre_key=nullptr;
    while( (*key) < (*node)  ){
      pre_key=key;
    ...
    //2.alter the next pointers
    if(key==node){
      //no need to alter the current position
      pre_node=node;
    }else{
      pre_node->set_next(post);
      if(pre_key==nullptr){
        p_head=node;
        node->set_next(key);
      }
      else{
        pre_key->set_next(node);
        node->set_next(key);
      }    
    }
 ...
 void History::update_acb_cgl(){
  ...
  while (node!=nullptr){
    ...
    cgl=0;
      ...
      share_ba=share_ba-(node->get_shares());
      acb_per_share=acb/share_ba; 
      cgl=(node->get_amount())-(node->get_shares())*acb_per_share;
    }
    node->set_acb(acb);
...
double History::compute_cgl(unsigned int year){
  ...
  while (node!=nullptr){
    if ((node->get_year())==year){
      cgl=cgl+(node->get_cgl());
    }//if the year is equal, just add the cgl of this node to the previous one.
    node=(node->get_next());
  }
  return cgl;
}
